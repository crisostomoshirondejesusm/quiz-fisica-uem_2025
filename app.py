...
# 2. Banco de Dados com todos os enunciados reais
if "perguntas" not in st.session_state:
    ...
    m_qs = [
        ...
        {"id": 30, "p": "Gráfico da função f. No ponto x=2, podemos concluir que:", "opts": ["A. lim f(x) à esquerda = f(2)", "B. lim f(x) não existe", "C. lim f(x) = 2", "D. lim f(x) à direita ≠ f(2)", "E. Nenhuma delas"], "c": "B", "img": "q30.png"},
    ...
    # Mesclagem Intercalada (Mat1, Fis1, Mat2, Fis2...)
    final_list = []
...
if not st.session_state.quiz_fim:
    ...
    # Identifica a matéria com base no ID original
    ...
    if q["img"]:
        ...
        if os.path.exists(img_path):
            ...
            st.warning(f"Aviso: Imagem {q['img']} não encontrada.")

    st.markdown(f"#### Pergunta (ID Original: {q['id']})")
    st.write(q['p'])
