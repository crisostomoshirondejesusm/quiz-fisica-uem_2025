...
# 2. Banco de Dados (Questões 41 a 80)
if "perguntas" not in st.session_state:
    st.session_state.perguntas = [
        ...
        {"id": 46, "p": "O gráfico representa a emissividade de corpos X, Y e Z. Qual dos corpos é o menos quente?", "opts": ["A. Ty=Tx", "B. Ty", "C. Tz", "D. Tz=Ty", "E. Tx"], "c": "B", "img": "Gráfico de Curvas de Radiação de Corpo Negro (Intensidade vs Comprimento de Onda)"},
        ...
        {"id": 55, "p": "No diagrama de níveis de energia, qual transição emite fotão com maior momento linear?", "opts": ["A. 1", "B. 2", "C. 3", "D. 4", "E. 5"], "c": "A", "img": "Esquema de Níveis de Energia (n=1 até n=infinito)"},
        ...
        {"id": 60, "p": "Radiação que atinge o ponto 3 no experimento do Bloco de Chumbo:", "opts": ["A. Infravermelha", "B. Ultravioleta", "C. alfa", "D. beta", "E. gama"], "c": "D", "img": "Diagrama de Radiação Alfa, Beta e Gama em Campo Magnético"},
        ...
        {"id": 63, "p": "Energia liberada (MeV) em fusão com defeito de massa de 0,02 uma?", "opts": ["A. 14,6", "B. 15,6", "C. 16,6", "D. 17,6", "E. 18,6"], "c": "E", "img": None},
        ...
        {"id": 66, "p": "Relação entre as vazões Q nas secções (1), (2) e (3) de uma tubulação?", "opts": ["A. Q1<Q2<Q3", "B. Q1>Q2>Q3", "C. Q1=Q2=Q3", "D. Q1+Q2=Q3", "E. Q1=Q2+Q3"], "c": "C", "img": "Tubulação horizontal com reduções de área"},
        ...
        {"id": 73, "p": "Denominações das etapas (1->2, 2->3, 3->1) no gráfico V vs T?", "opts": ["A. Isobárica, Isovolumétrica", "B. Isovolumétrica, Isobárica, Isotérmica", "C. Isotérmica, Isobárica", "D. Adiabática", "E. Cíclica"], "c": "B", "img": "Gráfico V(L) por T(K) mostrando ciclo termodinâmico"},
        ...
        {"id": 75, "p": "Qual representação de processos em gases ideais é FALSA nos gráficos p,V,T?", "opts": ["A", "B", "C", "D", "E"], "c": "E", "img": "Conjunto de Gráficos de Transformações Gasosas"},
        ...
        {"id": 78, "p": "Trabalho total (J) no ciclo XY-YZ-ZX do gráfico p vs V?", "opts": ["A. 1,6x10^5", "B. 2,0x10^5", "C. 3,2x10^5", "D. 4,8x10^5", "E. 0"], "c": "A", "img": "Gráfico de Pressão (Pa) versus Volume (m3)"},
        ...
        {"id": 80, "p": "Valor da amplitude de aceleração do corpo no gráfico MHS?", "opts": ["A. pi²", "B. 2pi²", "C. 3pi²", "D. 4pi²", "E. 5pi²"], "c": "B", "img": "Gráfico de Elongação versus tempo em MHS"}
    ]
...
# 5. Interface
st.title("🛡️ Simulado Física I - UEM 2025")

if not st.session_state.quiz_fim:
    # Cronômetro no topo
    t_restante = max(0, 5400 - int(time.time() - st.session_state.inicio_t))
    c1, c2 = st.columns(2)
    c1.metric("⏳ Tempo Restante", f"{t_restante//60}m {t_restante%60}s")
    c2.metric("📊 Questão", f"{st.session_state.i + 1} / 40")
    st.divider()

    q = st.session_state.perguntas[st.session_state.i]

    # Exibição segura da imagem/figura
    if q["img"]:
        st.warning(f"📌 [CONSULTE A FIGURA NO PDF: {q['img']}]")
        if q["id"] == 46:
            

[Image of Wien's displacement law graph]
    ...
    # Radio button com persistência de resposta
    marcada = st.session_state.respostas.get(st.session_state.i, None)
    idx_opt = 0
    if marcada:
        for i_opt, txt in enumerate(q["opts"]):
            if txt.startswith(marcada): idx_opt = i_opt

    escolha = st.radio("Selecione a opção:", q["opts"], index=idx_opt, key=f"rad_{st.session_state.i}")
    ...
    with col_nav1:
        if st.button("⬅️ ANTERIOR", use_container_width=True, disabled=(st.session_state.i==0)):
            st.session_state.i -= 1
            st.rerun()
    with col_nav2:
...
# 6. Resultados
    ...
    acertos = sum(1 for idx, q in enumerate(st.session_state.perguntas) if st.session_state.respostas.get(idx) == q["c"])
    nota = (acertos / total) * 20

    res1, res2 = st.columns(2)
    res1.metric("Acertos", f"{acertos} / {total}")
    res2.metric("Nota Final", f"{nota:.1f} / 20")
    ...
    if not st.session_state.ver_gabarito:
        if st.button("🔍 VER CORREÇÃO COMPLETA", use_container_width=True):
            st.session_state.ver_gabarito = True
            st.rerun()
    else:
        for idx, q in enumerate(st.session_state.perguntas):
            resp_dada = st.session_state.respostas.get(idx, "N/A")
            status = "✅" if resp_dada == q["c"] else "❌"
            with st.expander(f"Questão {q['id']}: {status}"):
                st.write(f"**Pergunta:** {q['p']}")
                st.write(f"Sua resposta: {resp_dada} | **Correta: {q['c']}**")

    if st.button("🔄 REINICIAR TODO O SIMULADO", use_container_width=True):
        reiniciar_exame()
