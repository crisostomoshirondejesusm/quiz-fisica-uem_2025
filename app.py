...
if "perguntas" not in st.session_state:
    st.session_state.perguntas = [
        ...
        {"id": 46, "p": "O gráfico representa a emissividade de corpos X, Y e Z. Qual dos corpos é o menos quente?", "opts": ["A. Ty=Tx", "B. Ty", "C. Tz", "D. Tz=Ty", "E. Tx"], "c": "B", "img": "Gráfico de Emissividade (Leis de Wien)"},
        ...
        {"id": 55, "p": "No diagrama de níveis de energia, qual transição emite fotão com maior momento linear?", "opts": ["A. 1", "B. 2", "C. 3", "D. 4", "E. 5"], "c": "A", "img": "Diagrama de Níveis de Energia de Bohr"},
        ...
        {"id": 66, "p": "Relação entre as vazões Q nas secções (1), (2) e (3) de uma tubulação?", "opts": ["A. Q1<Q2<Q3", "B. Q1>Q2>Q3", "C. Q1=Q2=Q3", "D. Q1+Q2=Q3", "E. Q1=Q2+Q3"], "c": "C", "img": "Diagrama de Tubulação Bernoulli"},
        ...
        {"id": 73, "p": "Denominações das etapas (1->2, 2->3, 3->1) no gráfico V vs T?", "opts": ["A. Isobárica, Isovolumétrica", "B. Isovolumétrica, Isobárica, Isotérmica", "C. Isotérmica, Isobárica", "D. Adiabática", "E. Cíclica"], "c": "B", "img": "Gráfico Volume vs Temperatura"},
        ...
        {"id": 75, "p": "Qual representação de processos em gases ideais é FALSA?", "opts": ["A", "B", "C", "D", "E"], "c": "E", "img": "Gráficos Termodinâmicos"},
        ...
        {"id": 78, "p": "Trabalho total (J) no ciclo XY-YZ-ZX do gráfico p vs V?", "opts": ["A. 1,6x10^5", "B. 2,0x10^5", "C. 3,2x10^5", "D. 4,8x10^5", "E. 0"], "c": "A", "img": "Gráfico Pressão vs Volume Ciclo"},
...
if not st.session_state.quiz_fim:
    # Cabeçalho de Status
    t_decorrido = int(time.time() - st.session_state.inicio_t)
    t_restante = max(0, 5400 - t_decorrido)
    
    col_t1, col_t2 = st.columns(2)
    col_t1.metric("⏳ Tempo", f"{t_restante//60}m {t_restante%60}s")
    col_t2.metric("📝 Questão", f"{st.session_state.i + 1} de 40")
    st.divider()
    ...
    if q["img"]:
        st.info(f"📍 Considere a imagem: **{q['img']}**")
        if q["id"] == 46:
...
[Image of Wien's displacement law graph]
    ...
    # Resposta
    id_atual = f"q_{st.session_state.i}"
    escolha = st.radio("Alternativas:", q["opts"], key=id_atual)
    ...
    with col_bt1:
        if st.button("⬅️ Anterior", disabled=(st.session_state.i == 0), use_container_width=True):
            ...
            st.rerun()
    with col_bt2:
        texto_botao = "Próxima ➡️" if st.session_state.i < 39 else "Finalizar 🏁"
        if st.button(texto_botao, use_container_width=True, type="primary"):
            st.session_state.respostas[st.session_state.i] = escolha[0] # Salva a letra (A, B, C...)
    ...
    # Auto-refresh simples para o cronômetro
    time.sleep(1)
    st.rerun()
...
# 5. Resultados
    ...
    nota = (acertos / total_perguntas) * 20

    c1, c2 = st.columns(2)
    c1.metric("Acertos", f"{acertos} / {total_perguntas}")
    c2.metric("Nota Final", f"{nota:.1f} / 20")

    if st.button("📸 Gerar Comprovativo (Print)", use_container_width=True):
    ...
    if not st.session_state.ver_gabarito:
        if st.button("🔍 Ver Gabarito Detalhado", use_container_width=True):
            st.session_state.ver_gabarito = True
            st.rerun()
    else:
        for idx, q in enumerate(st.session_state.perguntas):
            resp_usu = st.session_state.respostas.get(idx, "-")
            correto = "✅" if resp_usu == q["c"] else "❌"
            with st.expander(f"Questão {idx + 1}: {correto}"):
                st.write(f"**Pergunta:** {q['p']}")
                st.write(f"Sua resposta: {resp_usu} | Resposta correta: {q['c']}")
