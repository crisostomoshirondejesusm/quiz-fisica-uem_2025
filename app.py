...
if "perguntas" not in st.session_state:
    st.session_state.perguntas = [
        ...
        {"id": 46, "p": "O gráfico representa a emissividade de corpos X, Y e Z. Qual dos corpos é o menos quente?", "opts": ["A. Ty=Tx", "B. Ty", "C. Tz", "D. Tz=Ty", "E. Tx"], "c": "B", "img": "Gráfico de Curvas de Radiação (Lei de Wien)"},
        ...
        {"id": 55, "p": "No diagrama de níveis de energia, qual transição emite fotão com maior momento linear?", "opts": ["A. 1", "B. 2", "C. 3", "D. 4", "E. 5"], "c": "A", "img": "Diagrama de Transições Eletrónicas"},
        ...
        {"id": 60, "p": "Radiação que atinge o ponto 3 no experimento do Bloco de Chumbo:", "opts": ["A. Infravermelha", "B. Ultravioleta", "C. alfa", "D. beta", "E. gama"], "c": "D", "img": "Deflexão de partículas em Campo Magnético"},
        ...
        {"id": 66, "p": "Relação entre as vazões Q nas secções (1), (2) e (3) de uma tubulação?", "opts": ["A. Q1<Q2<Q3", "B. Q1>Q2>Q3", "C. Q1=Q2=Q3", "D. Q1+Q2=Q3", "E. Q1=Q2+Q3"], "c": "C", "img": "Tubulação com diferentes diâmetros"},
        ...
        {"id": 73, "p": "Denominações das etapas (1->2, 2->3, 3->1) no gráfico V vs T?", "opts": ["A. Isobárica, Isovolumétrica", "B. Isovolumétrica, Isobárica, Isotérmica", "C. Isotérmica, Isobárica", "D. Adiabática", "E. Cíclica"], "c": "B", "img": "Gráfico V-T"},
        ...
        {"id": 75, "p": "Qual representação de processos em gases ideais é FALSA?", "opts": ["A", "B", "C", "D", "E"], "c": "E", "img": "Gráficos de Processos Termodinâmicos"},
        ...
        {"id": 78, "p": "Trabalho total (J) no ciclo XY-YZ-ZX do gráfico p vs V?", "opts": ["A. 1,6x10^5", "B. 2,0x10^5", "C. 3,2x10^5", "D. 4,8x10^5", "E. 0"], "c": "A", "img": "Ciclo p-V"},
...
# 5. UI Principal
st.title("⚖️ Simulado de Física I - UEM 2025")

if not st.session_state.quiz_fim:
    # Cabeçalho: Tempo e Progresso
    t_restante = max(0, 5400 - int(time.time() - st.session_state.inicio_t))
    col1, col2 = st.columns(2)
    col1.metric("⏳ Tempo Restante", f"{t_restante//60}m {t_restante%60}s")
    col2.metric("📊 Progresso", f"{st.session_state.i + 1} / 40")
    st.divider()

    q = st.session_state.perguntas[st.session_state.i]

    # Exibição segura das referências visuais
    if q["img"]:
        st.info(f"💡 Referência Visual: {q['img']}")
        if q["id"] == 46:
...
[Image of Wien's displacement law graph]
    ...
    # Opções com persistência
    marcada = st.session_state.respostas.get(st.session_state.i, None)
    idx_selecionado = 0
    if marcada:
        for i_opt, texto in enumerate(q["opts"]):
            if texto.startswith(marcada):
                idx_selecionado = i_opt

    escolha = st.radio("Escolha a alternativa correta:", q["opts"], index=idx_selecionado, key=f"q_{st.session_state.i}")
...
# 6. Ecrã Final
    ...
    certas = sum(1 for idx, q in enumerate(st.session_state.perguntas) if st.session_state.respostas.get(idx) == q["c"])
    nota = (certas / 40) * 20

    res1, res2 = st.columns(2)
    res1.metric("Total de Acertos", f"{certas} / 40")
    res2.metric("Sua Nota", f"{nota:.1f} / 20")

    if st.button("📸 GUARDAR RESULTADOS (PRINT/PDF)", use_container_width=True):
    ...
    if not st.session_state.ver_gabarito:
        ...
        for idx, q in enumerate(st.session_state.perguntas):
            resp = st.session_state.respostas.get(idx, "-")
            cor = "✅" if resp == q["c"] else "❌"
            with st.expander(f"Pergunta {q['id']}: {cor}"):
    ...
    if st.button("🔄 REFAZER O EXAME", use_container_width=True):
        reiniciar_exame()
