...
if not st.session_state.quiz_fim:
    ...
    # --- BLOCO DE FIGURAS CORRIGIDO ---
    if quest["img"]:
        caminho_img = f"imagens/{quest['img']}"
        if os.path.exists(caminho_img):
            st.image(caminho_img, caption=f"Referência para a Questão {quest['id']}", use_container_width=True)
        else:
            st.warning(f"⚠️ Imagem {quest['img']} não encontrada na pasta 'imagens'.")
