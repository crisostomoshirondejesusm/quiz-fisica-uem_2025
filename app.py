...
if "perguntas" not in st.session_state:
    st.session_state.perguntas = [
        ...
        {"id": 46, "p": "O gráfico representa a emissividade de corpos X, Y e Z. Qual dos corpos é o menos quente?", "opts": ["A. Ty=Tx", "B. Ty", "C. Tz", "D. Tz=Ty", "E. Tx"], "c": "B", "img": "fig_46.png"},
        ...
        {"id": 55, "p": "No diagrama de níveis de energia, qual transição emite fotão com maior momento linear?", "opts": ["A. 1", "B. 2", "C. 3", "D. 4", "E. 5"], "c": "A", "img": "fig_55.png"},
        ...
        {"id": 60, "p": "Radiação que atinge o ponto 3 no experimento do Bloco de Chumbo:", "opts": ["A. Infravermelha", "B. Ultravioleta", "C. alfa", "D. beta", "E. gama"], "c": "D", "img": "fig_60.png"},
        ...
        {"id": 66, "p": "Relação entre as vazões Q nas secções (1), (2) e (3) de uma tubulação?", "opts": ["A. Q1<Q2<Q3", "B. Q1>Q2>Q3", "C. Q1=Q2=Q3", "D. Q1+Q2=Q3", "E. Q1=Q2+Q3"], "c": "C", "img": "fig_66.png"},
        ...
        {"id": 73, "p": "Denominações das etapas (1->2, 2->3, 3->1) no gráfico V vs T?", "opts": ["A. Isobárica, Isovolumétrica", "B. Isovolumétrica, Isobárica, Isotérmica", "C. Isotérmica, Isobárica", "D. Adiabática", "E. Cíclica"], "c": "B", "img": "fig_73.png"},
        ...
        {"id": 75, "p": "Qual representação de processos em gases ideais é FALSA?", "opts": ["A", "B", "C", "D", "E"], "c": "E", "img": "fig_75.png"},
        ...
        {"id": 78, "p": "Trabalho total (J) no ciclo XY-YZ-ZX do gráfico p vs V?", "opts": ["A. 1,6x10^5", "B. 2,0x10^5", "C. 3,2x10^5", "D. 4,8x10^5", "E. 0"], "c": "A", "img": "fig_78.png"},
        ...
        {"id": 80, "p": "Valor da amplitude de aceleração do corpo no gráfico MHS?", "opts": ["A. pi²", "B. 2pi²", "C. 3pi²", "D. 4pi²", "E. 5pi²"], "c": "B", "img": "fig_80.png"}
    ]
...
# 4. Função de Reiniciar (Limpeza Total)
def reiniciar_exame():
    for key in ["i", "respostas", "quiz_fim", "ver_gabarito", "inicio_t"]:
...
if not st.session_state.quiz_fim:
    # Tempo e Progresso
    tempo_restante = max(0, 5400 - int(time.time() - st.session_state.inicio_t))
    st.sidebar.metric("⏳ Tempo Restante", f"{tempo_restante//60}m {tempo_restante%60}s")
    
    idx = st.session_state.i
    quest = st.session_state.perguntas[idx]
    
    st.subheader(f"Questão {idx + 1} de 40")
    
    # Exibir Imagem se houver
    if quest["img"]:
        # Tenta carregar a imagem, se não existir mostra placeholder
        try:
            st.image(quest["img"], caption=f"Figura da Questão {quest['id']}")
        except:
            st.warning(f"🖼️ [Figura {quest['id']} do PDF - Gráfico/Diagrama]")
            

    st.info(quest["p"])
    
    # Recuperar resposta se já marcou
...
# 6. Tela Final
    ...
    col1.metric("Acertos", f"{acertos} / 40")
    col2.metric("Nota", f"{(acertos/40)*20:.1f} / 20")

    if st.button("📸 CAPTURAR RESULTADOS (PRINT)", use_container_width=True, type="primary"):
    ...
    if not st.session_state.ver_gabarito:
        if st.button("🔍 VER CORREÇÃO E GABARITO", use_container_width=True):
        ...
        for i, q in enumerate(st.session_state.perguntas):
            ...
            simbolo = "✅" if sua == q["c"] else "❌"
            with st.expander(f"Questão {q['id']}: {simbolo}"):
                st.write(f"**Pergunta:** {q['p']}")
                st.write(f"Sua resposta: {sua} | Correta: {q['c']}")
        
        if st.button("⬆️ FECHAR GABARITO", use_container_width=True):
